...
public class Problem1431 {
    ...
    * intuition is to find max among the array and iterate over array and check if extra+cur >= max
    if so true else false
    algo:
    * init max = candies[0]
    * iterate over candies and update max
    * create ans list
    * iterate over candies and if cand+exta >= max then add true else false to ans
    ...
    * it takes n time and const space
    ...
    public List<Boolean> kidsWithCandies(int[] candies, int extraCandies) {
        return kidsWithCandiesA1(candies, extraCandies);
    }

    public List<Boolean> kidsWithCandiesA1(int[] candies, int extraCandies) {
        int max=candies[0];
